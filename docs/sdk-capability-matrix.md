...
| CLI probe readiness | Removed as SDK readiness source | ADR 0002 rejects terminal probing as the production execution boundary. | `PreflightSdkRuntime` rejects `CLI_PROBE` capability sources; legacy CLI probe code has been removed. | Must not be used as production-equivalent SDK readiness evidence. |
