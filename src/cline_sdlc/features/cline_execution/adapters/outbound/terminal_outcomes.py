"""Parse schema-versioned terminal outcomes from Cline output streams."""
...
class ParsedTerminalOutcomes:
    """Validated terminal outcomes plus non-parseable output lines."""

    outcomes: tuple[SessionOutcome, ...]
    malformed_lines: tuple[str, ...]
    non_outcome_lines: tuple[str, ...]


def parse_terminal_outcomes(stdout: str) -> ParsedTerminalOutcomes:
    """Parse terminal outcomes while preserving human or malformed output lines."""
    outcomes: list[SessionOutcome] = []
    malformed_lines: list[str] = []
    non_outcome_lines: list[str] = []
    for line in stdout.splitlines():
        ...
        candidates = _candidate_outcomes(value)
        if not candidates:
            non_outcome_lines.append(line)
            continue
        for candidate in candidates:
            try:
                ...
                malformed_lines.append(line)
    return ParsedTerminalOutcomes(
        outcomes=tuple(outcomes),
        malformed_lines=tuple(malformed_lines),
        non_outcome_lines=tuple(non_outcome_lines),
    )


def timeout_output(value: str | bytes | None) -> str:
    """Return timeout-captured process output as replacement-safe text."""
    if value is None:
        return ""
